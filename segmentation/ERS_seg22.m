...
%
function [timg labels]=ERS_seg22(img,min_num,nC)
close all;
%clear all;clc
timg=[];
...
grey_img = double(img);%double(rgb2gray(img));%img;%
...
% nC = 1200;%round(size(img,1)*size(img,2)/min_num);%500indian%Pavia 1200 w-dc1500 salinas300
%// Call the mex function for superpixel segmentation\
%// !!! Note that the output label starts from 0 to nC-1.
t = cputime;

lambda_prime = 0.2;sigma = 8.0; %ԭʼlambda_prime = 0.5;sigma = 5.0; %indian 0.2;sigma = 8.0
