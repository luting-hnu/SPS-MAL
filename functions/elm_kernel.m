...
if Elm_Type~=REGRESSION
    ...
    sorted_target=sort(cat(2,T,TV.T),2);            %cat(2,A,B) 's result is[A,B].sort(*,2) sort the array by a incremental order.
    label=zeros(1,1);                               %   Find and save in 'label' class label from training and testing data sets
    label(1,1)=sorted_target(1,1);
    j=1;
    for i = 2:(NumberofTrainingData+NumberofTestingData)
        if sorted_target(1,i) ~= label(1,j)         %No "!=" in maltlab.
            j=j+1;
            label(1,j) = sorted_target(1,i);
        end          %If the later elment differs from it,then save the number in its last position.
    end
    number_class=j;
